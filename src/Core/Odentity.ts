...
export class Odentity {
  ...
  static async init(threads: Threads): Promise<Odentity> {
    if (this._instance == undefined) {
      ...
      let odentityCollection = await odentityThread.getCollection<
        OdentityEntity
      >("odentity");
      let providerCollection = await odentityThread.getCollection<
        OdentityProvider
      >("odentityProvider");
      let loginReqCollection = await odentityThread.getCollection<LoginRequest>(
        "loginRequest"
      );
      this._instance = new Odentity(
        restored,
        odentityCollection,
        providerCollection,
        loginReqCollection
  ...
  static async createSchemas(threads: Threads): Promise<Odentity> {
    if (this._instance == undefined) {
      ...
      let odentityCollection = await odentityThread.getOrCreateCollection<
        OdentityEntity
      >("odentity", OdentitySchema);
      let providerCollection = await odentityThread.getOrCreateCollection<
        OdentityProvider
      >("odentityProvider", OdentityProviderSchema);
      let loginReqCollection = await odentityThread.getOrCreateCollection<
        LoginRequest
  ...
  private constructor(
    odentity: OdentityEntity | null,
    odentityCollection: RemoteCollection<OdentityEntity>,
    providerCollection: RemoteCollection<OdentityProvider>,
    loginReqCollection: RemoteCollection<LoginRequest>
  ) {
  ...
  async login(reference: string, type: string, callback: any) {
    var odentityProvider = await this.createOdentityProviderIfNotExist(
      reference,
      "email"
    );
    ...
    if (odentity != null) {
      ...
      this.loginRequestCollection.observeUpdate(
        ["SAVE"],
        request._id,
        async (request: LoginRequest) => {
          if (odentityProvider.odentityId) {
            var odentity = await this.odentityCollection.findById(
              odentityProvider.odentityId
            );
            localStorage.setItem(Odentity.STORAGE, JSON.stringify(odentity));
            this._current = odentity;
            callback(request);
  ...
  private async createOdentityProvider(
    odentity: OdentityEntity,
    identityReference: string,
    type: string
  ): Promise<OdentityProvider> {
  ...
  private async createOdentity(): Promise<OdentityEntity> {
    ...
    odentity.cryptoIdentity = (
      await Libp2pCryptoIdentity.fromRandom()
    ).toString();
    odentity = await this.odentityCollection.create(odentity);
    return odentity;
  }

  private async createOdentityProviderIfNotExist(
    identityReference: string,
    type: string
  ): Promise<OdentityProvider> {
    var query: QueryJSON = {
    ...
    if (instances.length == 0) {
      ...
      return await this.createOdentityProvider(
        odentity,
        identityReference,
        type
      );
  ...
  async connectCircleWallet(safeOwner: Account, safeAddress: string) {
    if (this._current == null) throw new Error("You are not logged in");
