...
export class OdentityStore {
  ...
  async currentOmo(): Promise<Omo | null> {
    if (this._currentOmo != null) return this._currentOmo;
    ...
    if (omoid != null && omoid != "") {
      try {
        ...
        this._currentOmo = (
          await client.findByID<Omo>(this.threadId, "Omo", omoid)
        ).instance;
  ...
  async login(reference: string, type: string, callback: any) {
    ...
    var identityProvider = await this.createOmoIfNotExist(
      reference,
      "email",
      client
    );
    if (identityProvider == null)
      throw new Error("omo couldn't be found or created");
      ...
      this.hub.observeUpdate(
        ...
        async (request: LoginRequest) => {
          if (identityProvider != undefined) {
            debugger;
            this._currentOmo = (
              await client.findByID<Omo>(
                this.threadId,
                "Omo",
                identityProvider.omoid || ""
              )
            ).instance;
  ...
  private async getLoginRequest(id) {
    var client = await this.hub.getClient();
    return (
      await client.findByID<LoginRequest>(this.threadId, "LoginRequest", id)
    ).instance;
  }
  ...
  private async init() {
    ...
    if (!(await this.hub.hasCollection(this.threadId, "Omo", client)))
      await this.hub.newCollection<Omo>(
        this.threadId,
        "Omo",
        OmoSchema,
        client
    ...
    if (
      !(await this.hub.hasCollection(this.threadId, "IdentityProvider", client))
    )
      await this.hub.newCollection<IdentityProvider>(
        this.threadId,
        "IdentityProvider",
        IdentityProviderSchema,
        client
    ...
    if (!(await this.hub.hasCollection(this.threadId, "LoginRequest", client)))
      await this.hub.newCollection<LoginRequest>(
        this.threadId,
        "LoginRequest",
        LoginRequestSchema,
        client
  ...
  private async createLoginRequest(identityProvider: IdentityProvider) {
    ...
    var response = await client.create(this.threadId, "LoginRequest", [
      request,
    ]);
  ...
  async acceptLoginRequest(id, invite) {
    ...
    var request = (await client.findByID(this.threadId, "LoginRequest", id))
      .instance;
  ...
  private async createIdentityProvider(
    ...
    var response = await client.create(this.threadId, "IdentityProvider", [
      provider,
    ]);
  ...
  private async createOmoIfNotExist(
    identityReference: string,
    type: string,
    client: Client
    ...
    var instances = (
      await client.find<IdentityProvider>(
        this.threadId,
        "IdentityProvider",
        query
      )
    ...
    if (instances.length == 0) {
      ...
      return await this.createIdentityProvider(
        omo,
        identityReference,
        type,
        client
