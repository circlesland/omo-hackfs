...
export class Quantum {
  private static threadName = "QUANTA3";
  ...
  private constructor(
    quantaCollection: OmoCollection<Quant>,
    threadId: ThreadID
  ) {
  ...
  static async initQuantum(): Promise<Quantum> {
    ...
    var quantaCollection = await OmoCollection.createOmoCollection<Quant>(
      Quantum.threadName,
      QuantSchema,
      threadId,
      client
  ...
  async seedDb() {
    ...
    modelHelper.modelQuanta.forEach(async (quant) => {
      await OmoCollection.createOmoCollection(
        quant.collectionName,
        quant.toTextileSchema(),
        this.threadId,
        client
      );
    });
  ...
  async getCollection<T>(collectionName: string): Promise<OmoCollection<T>> {
    var collection = await this.quantaCollection.find<any>({
      ands: [
        { fieldPath: "collectionName", value: { string: collectionName } },
      ],
    });
    if (collection.length != 1)
      throw new Error("No ore multiple collection found");
    return OmoCollection.byName<T>(
      collectionName,
      this.threadId,
      JSON.parse(collection[0].jsonSchema)
    );
  }
  ...
  private createSubscription(
    ...
    if (this.listened.some((x) => x == collectionName)) return;
    client.listen(
      this.threadId,
      [
        {
