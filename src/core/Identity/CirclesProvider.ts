...
export class CirclesProvider implements IdentityProviderInterface {
  async login(loginRequest: LoginRequest, providerIdentity: OdentityProvider): Promise<OdentityEntity | null> {
    ...
    if (providerIdentity.odentityId == null || providerIdentity.externalReference == null) return null;

    var odentity = await odentityCollection.findById(providerIdentity.odentityId);
    ...
    odentity.circleSafeOwner = { address: safeOwner.address, privateKey: safeOwner.privateKey };
