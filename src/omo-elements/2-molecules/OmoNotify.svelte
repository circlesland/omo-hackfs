...
{#if show}
  ...
  <div
    ...
    h-screen">
    <ul class="px-3 space-y-3 h-full overflow-y-scroll overflow-y-hidden">
      {#each notifications as item}
        {#if item.tag == 'Symbol(ADD_CONNECTION)'}
          <div class="flex w-sm mb-4 bg-blue-300">
          ...
          <!-- <div
          class="bg-gray-100 border-l-4 border-red p-4 py-6 rounded shadow-lg
          flex items-center justify-between mb-6"
          role="alert">
          <span class="fa-stack fa-2x sm:mr-2 mb-3">
            <i class="fas fa-circle text-red-dark fa-stack-2x" />
            <i class="fas fa-hand-paper fa-stack-1x text-white" />
          </span>
          <div class="sm:text-left text-center sm:mb-0 mb-3 w-128">
            <p class="font-bold mb-1 text-lg">{item.tag}</p>
            <p class="text-grey-dark inline-block">{item.text}</p>
          </div>
          <i class="fas fa-times mx-4 fa-2x text-grey-darker" />
        </div> -->
          <!-- <li class="p-3 border rounded shadow-md bg-gray-100">
          <div class="mb-1">
            <span
              class="inline-flex text-white rounded-full h-6 px-3 justify-center
              items-center"
              class:bg-secondary={item.tag === 'Symbol(TRANSFER)'}
              class:bg-green-500={item.tag === 'Symbol(ADD_CONNECTION)'}
              class:bg-red-500={item.tag === 'Symbol(REMOVE_CONNECTION)'}>
              {item.tag}
            </span>
          </div>
          <p>{item.text}</p>
