...
<div class="max-w-sm bg-white shadow-lg rounded-lg overflow-hidden my-4">
  <span
    ...
    uppercase text-white bg-leap5">
    {data.follower} / 196.417
  </span>
  <div class="relative">
    <div class="overflow-hidden h-3 text-xs flex bg-dark">
      <div
        style="width: {(data.follower * 100) / 196417}%"
  ...
  <img
    ...
    src={data.image}
    alt={data.name} />
  <div class="w-full flex items-center px-6 py-3 bg-dark">
    <div
      class="w-full flex justify-between font-title text-white font-semibold
      text-lg">
      <span>
        Ø{(data.price - (data.price / 100) * data.discount).toFixed(2)} / week
      </span>
      <span class="text-red-500">
        -{data.discount}%
        <span class="px-1 line-through">Ø{data.price.toFixed(0)}</span>
      </span>
    </div>
  </div>
  <div class="py-4 px-6">
    <h1 class="text-2xl font-semibold text-leap5">{data.name}</h1>
    <p class="py-2 text-lg text-gray-700 h-32">{data.description}</p>
    <div class="flex items-center mt-4 text-gray-700">
      <svg class="h-6 w-6 fill-current" viewBox="0 0 512 512">
        <path
          d="M239.208 343.937c-17.78 10.103-38.342 15.876-60.255 15.876-21.909
        ...
        <g>
          <path
            d="M322.602 384H480c-10.638-42-39.537-81.691-86.703-96.072-17.781
      ...
      <p class="px-2 text-sm">{data.group}</p>
    </div>
    <div class="flex items-center mt-4 text-gray-700">
      <svg class="h-6 w-6 fill-current" viewBox="0 0 512 512">
        <path
          d="M256 32c-88.004 0-160 70.557-160 156.801C96 306.4 256 480 256
...
<section class="border border-gray-100">
  ...
  <div class="relative">
    <div class="overflow-hidden h-3 text-xs flex bg-gray-400">
      <div
        style="width: {(data.follower * 100) / 1596}%"
        class="shadow-none flex flex-col text-center whitespace-nowrap
        text-white justify-center bg-tertiary" />
    </div>
  </div>

  <img class="h-56 w-full object-cover" src={data.image} alt={data.name} />
  <div class="bg-gray-100 px-4 py-2">
    <h1 class="text-gray-900 font-bold text-3xl uppercase text-primary">
      {data.name}
    </h1>
  ...
  <div class="font-title flex justify-between bg-leap3">
    <div class="px-4 py-2 text-white font-bold text-lg">
      Ø{data.price} / week
    </div>
    <a
      href="javascript:navigate('omodream')"
      class="text-center text-white bg-tertiary font-bold text-lg px-4 py-2
      uppercase hover:bg-secondary">
      pre-order now
    </a>
  </div>
</section> -->
