...
export const checkAuthentification: ISideEffect<IProcessContext, any> = {
  ...
  execute: async (context, argument) => {
    ...
    const ownerId = (await Omosapiens.byId(dream.data.DreamById.creatorId)).odentityId;
    let triesLeft = 6;
    ...
    if (!window.o.odentity.current || !window.o.odentity.current.circleSafe) throw new Error("Not logged in");
    ...
    while (!isTrusted && triesLeft-- >= 0) {
      isTrusted = await checkTrust(trustReceivingSafeOwner, trustReceivingSafeAddress);
    }
    if (!isTrusted) {
